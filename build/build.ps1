# Construye el ejecutable de FichajeLocal con PyInstaller (onedir).
...
Push-Location $root
& $py -m PyInstaller --noconfirm --clean (Join-Path $PSScriptRoot "FichajeLocal.spec")
$code = $LASTEXITCODE
Pop-Location

if ($code -eq 0) {
    Write-Host "`n== LISTO ==" -ForegroundColor Green
    Write-Host "Ejecutable: $(Join-Path $root 'dist\FichajeLocal\FichajeLocal.exe')"
} else {
